...
提供 CL/BZ 价差网格、CL/BZ 库存组合，以及 **Lighter QQQ 只做多挂单剥头皮 + Variational US100 对冲**三种独立纸面策略。QQQ 模式对照三种格距与三种对冲阈值，共九组，单组 30 格、每格 1000 USDC；部署和统计口径见文末。
...
**仅获取公开行情不需要令牌。** 2026-09-19 无 Cookie 实测：前端 K 线、合约元数据以及官方公开 `GET /metadata/stats` 均返回 200，CL/BZ 的 `POST /api/quotes/indicative` 返回 403；同一报价接口使用现有会话成功。CL/BZ 模式选择后一种按数量报价，并在启动时检查登录状态，所以仍需令牌。官方公开统计也有买卖价，但按固定名义金额分档，文档允许最长 600 秒缓存；不能直接当成按当前每腿桶数取得的实时成交报价。QQQ 模式使用另一条无需登录的按数量报价接口，详见文末。
...
## Lighter QQQ 剥头皮与 US100 对冲：九组

这套模式只在 Lighter QQQ 模拟挂买单建多仓，每笔持仓挂卖单止盈；Variational US100 用于按需模拟空头对冲。它有独立参数、账本和面板，保留已有 CL/BZ 实验。配置见 [qqq-hedge.example.json](qqq-hedge.example.json)，`kind` 为 `qqq_hedge`。

| 参数 | 默认设置 |
| --- | --- |
| QQQ 方向 | 只做多 |
| 网格数量 | 每组 30 格 |
| 每格名义金额 | 1000 USDC，数量按该格限价与交易步长取整 |
| 网格间距对照 | 0.05%、0.1%、0.2% |
| 对冲阈值对照 | 0%、2%、5% |
| 场景总数 | 三种间距 × 三种阈值，共九组 |
| 每笔止盈 | 自该格入场价向上一格的价格目标 |
| 对冲后目标 | 所选阈值的一半，按 US100 数量限制舍入 |
| 对冲映射 | `beta=1` 的美元等值近似 |
| 收益口径 | 未计实际资金费、隔夜费及股息调整，`before_funding_and_dividends` |

每轮以首次有效 QQQ 中间价为固定锚点，向下布置 30 层买单；止盈价为各层入场价乘以 `1 + 格距比例`。已有仓位不随报价移动重设成本或止盈价，部分入场成交也可对应挂出止盈单，止盈完成后该层重新挂回原入场价。整体空仓且价格向上离开锚点至少一格时，先撤旧挂单，等待 300 毫秒的模拟撤单延迟后再设新锚点。每格 1000 USDC 是计划买入金额，部分成交和数量取整后实际金额可能较小。30 格表示最多 30 份 QQQ 网格库存，满格计划入场金额合计约 30,000 USDC；US100 对冲名义金额另计。

QQQ 成交由公开逐笔与盘口排队模型驱动：先消耗模拟订单前方的可见排队量，再按剩余数量记录部分成交。仅触价不会直接记成交，改价重挂会重新排队，卖出止盈仍需满足 Maker 成交条件。重连、逐笔缺口和停机期间不会补造历史成交；页面记录暂停或缺口状态。同一种格距的三种对冲阈值使用相同的 QQQ 挂单与成交路径，阈值差异仅影响 US100 对冲，从而能比较对冲频率、成本和收益。

对冲比例按两平台**实际净持仓**对应的风险名义金额计算：

```text
L = beta × QQQ 净数量 × QQQ 价格 × QQQ 合约乘数
U = US100 净数量 × US100 价格 × US100 合约乘数
净敞口比例 = abs(L + U) / (abs(L) + abs(U)) × 100%
```

多头数量为正，空头为负；双方空仓时比例为 0%。它不是初始资金或 30,000 USDC 网格预算的百分比。第一笔 QQQ 多仓成交且尚未对冲时比例为 100%，因此会超过九组中任一阈值。只有**严格超过**所选比例才触发调整，调整目标为 0%、1% 或 2.5%；数量步长和最小下单量可能留下残差，面板显示实际结果。QQQ 止盈后若 US100 空头过多，也会按同一规则减少对冲。不得把 QQQ 数量直接作为 US100 数量：二者价格、计价单位和跟踪误差均不同，`beta=1` 只表示美元风险近似。

QQQ 估值与美元敞口使用新鲜盘口中价，US100 使用指示性报价中的 mark。默认手续费均为 0，US100 在指示性买卖价之外另加 1 bps 滑点；这两项均可配置。行情和摘要在本地压缩保存，保留完整收益历史与成交记录；账户只保存恢复和发布所需的最近两个检查点。

面板分别显示 Lighter 与 Variational 的损益、净数量、累计成交量、USDC 成交额，并显示组合损益、净敞口比例、阈值与对冲次数。平台原始成交数量各用自己的合约单位，不能跨平台相加；跨平台比较使用 USDC 成交额。成交统计只计账本实际记录的纸面执行，不计挂单金额和报价请求。手续费、成交点差与配置滑点按各平台口径计算；一格止盈是价格目标，不是扣费后净收益承诺。

QQQ 为 Lighter 上的合约行情，US100 为 Variational 对应市场，两者可能存在基差、交易时段和报价可用性差异。程序使用市场状态与新鲜报价约束模拟。Variational 使用无需登录的 `POST /api/quotes/simple`，获取对应数量的双边指示性报价、时间和数量限制；这些报价不是成交保证。公共统计接口可能缓存 600 秒，不用于替代此报价。**QQQ 模式不需要令牌、Lighter 交易账户、API 私钥或钱包私钥，也不读取登录会话文件。**模拟不计实际资金费、隔夜费及股息调整，也不复制交易所强平引擎。

HTTP 429 表示公共接口限流。九组共用串行报价预算，相同数量只请求一次，待报价账户轮流获得请求机会。每轮最多一个实际对冲数量报价；本轮需要刷新公共估值时，允许先取探针、紧接着取一个实际数量报价，最多两次 POST，避免探针一直占用额度。每次 POST 占用 3 秒请求预算，遇到限流后占用 6 秒；连续两次会累计占用预算，后续轮次须等待额度恢复。批次内也逐次检查限流，不绕过冷却。这是程序的保守请求节奏，不是平台公布的限额。

公共估值报价在原始时间起最多 9 秒内复用，实际模拟对冲仍需要本轮新请求的对应数量报价；Var 元数据最多在本地缓存 30 秒，且受源时间与休市时间约束。两个平台分别冷却：遵守 `Retry-After` 的秒数或日期，至少等 60 秒；连续 429 按 60/120/240/480/900 秒退避，服务端要求更久时等待更久。冷却期间暂停新网格入场，保留已有仓位和已确认的 QQQ 模拟成交；不得用过期价格补造对冲。页面显示平台与重试倒计时。冷却保存在 `market-cooldowns.json`，重启、升级或重置账户均不会跳过等待；策略参数与账本身份不变，升级无需重置实验。

Linux 服务器一条命令安装或切换到九组模拟：

```bash
curl -fsSL https://raw.githubusercontent.com/hxx344/variational-cl-bz-grid/main/install.sh | sudo bash -s -- --qqq-hedge
```

首次创建 `/etc/variational-grid/qqq-hedge.json`，数据存入 `/var/lib/variational-grid/qqq-hedge/`。其中 `base_config` 保留对 `/etc/variational-grid/config.json` 的配置引用；QQQ 网格、成本和对冲参数由新配置独立保存，不继承 CL/BZ 经济参数，也不读取会话或执行旧网格迁移。首次安装无需粘贴令牌。已有配置和账本不覆盖，重复执行保留九组设置、复用验证缓存，并只在相关代码或设置变化时重启服务。不带参数时继续上次保存的模式；使用 `--compare`、`--inventory` 或 `--single` 可切换回原模式，返回时再次使用 `--qqq-hedge`。

网页复用 `variational-grid-web.service`，监听服务器 `127.0.0.1:9876`。在自己的电脑保持以下 SSH 转发，浏览器打开 [QQQ / US100 监控页](http://127.0.0.1:18765/)：

```powershell
ssh -N -o ExitOnForwardFailure=yes -L 18765:127.0.0.1:9876 root@你的服务器IP
```

已下载仓库时，可分别在两个终端启动模拟和网页，无需导入会话：
...
服务器上执行状态或重置命令时，将 `--experiments` 改为 `/etc/variational-grid/qqq-hedge.json`。网页重置与命令重置均先归档九组账本，再按相同参数重新开始；不记作平仓成交，不修改会话。修改格距、成本、阈值等经济参数时须改用新的 `output_dir`，以免混合不同口径结果。

无需网络和令牌的确定性演示：

```powershell
python -m variational_grid qqq-demo --output output/qqq-demo
```

使用新的输出目录运行；该演示只验证合成行情下的挂单、对冲与记账行为，不是历史回测或真实收益预测。
