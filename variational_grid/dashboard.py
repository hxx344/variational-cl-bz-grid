"""Loopback-only, read-only dashboard; never reads a login token or calls the venue."""
...
def read_history(db, summary, window):
    ...
    columns = ["ts", "json_extract(payload,'$.center_7d')", "json_extract(payload,'$.scenarios[0].spread_bz_minus_cl')"]
...
def make_server(experiment, port=9876):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self, head=False):
            ...
            try:
                ...
                if parsed.path == "/api/dashboard":
                    ...
                    payload = json.dumps(read_dashboard(experiment, window), ensure_ascii=False).encode()
        ...
        def do_POST(self):
            self.reply(405, b"Read-only monitor")
...
def serve_dashboard(args):
    ...
    with make_server(experiment, args.port) as server:
        emit({"event": "dashboard", "url": f"http://127.0.0.1:{server.server_port}", "read_only": True})
