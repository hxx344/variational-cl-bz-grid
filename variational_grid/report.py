...
def render_report(summary, recent, runtime):
    ...
    if not summary:
        ...
        content += '<h2>参数与结果</h2><p>各组资金独立，使用相同采样时刻；总盈亏包含未平仓持仓的退出估值。尚不足以判断长期优劣。</p><div class="table-wrap" tabindex="0" role="region" aria-label="网格配置对照表，可横向滚动"><table><thead><tr><th>配置</th><th>网格间距<br>当前一格 USDC/桶</th><th>每腿桶数<br>最多组数</th><th>初始资金<br>USDC</th><th>总盈亏<br>USDC</th><th>已实现<br>USDC</th><th>最大回撤</th><th>已平 / 已开组</th><th>当前持仓组</th><th>保证金 / 峰值<br>USDC</th><th>累计成交额<br>USDC</th><th>累计成交量<br>桶 · 双腿开平仓</th><th>状态</th></tr></thead><tbody>'
        for r in rows:
