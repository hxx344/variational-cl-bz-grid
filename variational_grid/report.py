...
def render_report(summary, recent, runtime):
    ...
    if not summary:
        ...
        content = f'<section class="market"><div><span>当前 BZ − CL</span><strong>{number(spread,4)}<small> USDC/桶</small></strong></div><div><span>{summary.get("center_window_hours",168)//24} 日中枢</span><strong>{number(summary.get("center",summary.get("center_7d")),4)}<small> USDC/桶</small></strong></div><div><span>最新采样 · UTC</span><strong class="date">{escape(summary["time_utc"])}</strong></div></section>'
        content += '<h2>参数与结果</h2><p>各组资金独立，使用相同采样时刻；总盈亏包含未平仓持仓的退出估值。尚不足以判断长期优劣。</p><div class="table-wrap" tabindex="0" role="region" aria-label="网格配置对照表，可横向滚动"><table><thead><tr><th>配置</th><th>网格间距<br>当前一格 USDC/桶</th><th>覆盖范围</th><th>每腿桶数<br>每侧网格层数</th><th>初始资金<br>USDC</th><th>总盈亏<br>USDC</th><th>已实现<br>USDC</th><th>最大回撤</th><th>已平 / 已开组</th><th>当前持仓组</th><th>保证金 / 峰值<br>USDC</th><th>开仓金额额度</th><th>估算杠杆</th><th>累计成交额<br>USDC</th><th>累计成交量<br>桶 · 双腿开平仓</th><th>状态</th></tr></thead><tbody>'
        for r in rows:
            state = "回撤停机" if r["halted"] else "只减仓" if not r["open_allowed"] else "正常"
            spacing = (number(r["grid_step_percent"], 1) + "%<br>") if r.get("grid_step_percent") is not None else ""
            coverage = f'上下各 {number(r["grid_range_percent"],1)}%<br>总跨度 {number(r["grid_span_percent"],1)}%' if r.get("grid_range_percent") is not None else "—"
            if r.get("grid_limit_enabled") is False or r.get("max_levels", 0) is None:
                coverage = "无固定覆盖范围"
            levels = "格数不限" if r.get("max_levels", 0) is None else str(r.get("max_levels", "—"))
            leverage = number(r["paper_leverage"],0) + " 倍" if r.get("paper_leverage") is not None else "—"
            limit = '金额不限' if r.get('margin_limit_enabled') is False else (number(r['entry_notional_limit_usdc'],2) + ' USDC' if r.get('entry_notional_limit_usdc') is not None else '—')
            turnover = number(r["turnover_usdc"], 2) if "turnover_usdc" in r else "—"
            volume = number(r["volume_barrels"], 3) if "volume_barrels" in r else "—"
            content += f'<tr><th scope="row">{escape(r["name"])}</th><td>{spacing}{number(r["grid_step"],6)}</td><td>{coverage}</td><td>{number(r["quantity_barrels"])} / {levels}</td><td>{number(r["initial_balance_usdc"],2)}</td><td class="pnl">{number(r["total_pnl_usdc"],4)}</td><td>{number(r["realized_pnl_usdc"],4)}</td><td>{number(dec(r["max_drawdown_fraction"])*100,3)}%</td><td>{r["closed_pairs"]} / {r["opened_pairs"]}</td><td>{r["open_pairs"]}</td><td>{number(r["margin_usdc"],2)} / {number(r["max_margin_usdc"],2)}</td><td>{limit}</td><td>{leverage}</td><td>{turnover}</td><td>{volume}</td><td>{state}</td></tr>'
        ...
        content += f'</ul><p>中枢：{escape(summary["history_start_utc"])} 至 {escape(summary["history_end_utc"])}，{summary.get("center_window_hours",168)} 根对齐、已收盘小时 K 线。最大回撤、峰值保证金及双腿开平仓成交量从本次实验首个有效采样起累计。百分比以中枢绝对值为基准，净止盈按开仓时的一格固定；成交额按成交桶数乘成交价累计，不含手续费。</p></details>'
