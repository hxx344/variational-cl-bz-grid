...
def render_report(summary, recent, runtime):
    ...
    if not summary:
        ...
        content += f'</ul><p>中枢：{escape(summary["history_start_utc"])} 至 {escape(summary["history_end_utc"])}，168 根对齐、已收盘小时 K 线。最大回撤、峰值保证金及双腿开平仓成交量从本次实验首个有效采样起累计。百分比以中枢绝对值为基准，净止盈按开仓时的一格固定；成交额按成交桶数乘成交价累计，不含手续费。</p></details>'
