...
class Engine:
    def __init__(self, config, store):
        ...
        self.step = dec(config.grid_step_usdc_per_barrel)
    ...
    def tick(self, center, cl, bz, now, *, allow_open=True):
        ...
        depth = min(int(abs(deviation) / self.step), self.config.max_levels)
        actions = []
        with self.store.transaction():
            ...
            for lot in self.store.lots():
                ...
                if not reason and now - lot["opened"] >= self.config.max_holding_hours * HOUR:
                    reason = "max_holding"
                if not reason and net >= self.qty * self.step:
                    reason = "take_profit"
                if reason:
            ...
            snapshot = {
                ...
                "skip_reason": skip_reason, "actions": actions,
            }
